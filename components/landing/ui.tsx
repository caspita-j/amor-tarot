...
export function Accent({ children }: { children: ReactNode }) {
  return <span className="font-bold text-[1.08em] text-[var(--accent)]">{children}</span>;
}

/* ── <Kicker> — caps 12px/600 tracking +0.08em en acento (máx 1 por sección) ── */
export function Kicker({ children }: { children: ReactNode }) {
  return (
    <p className="mb-3 flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.08em] text-[var(--accent)]">
      {/* Mini-aro: repite el dispositivo ownable del hero (el aro medidor de
          FICHA-ARTE.md) como micro-detalle de marca en cada sección. */}
      <svg width="12" height="12" viewBox="0 0 12 12" aria-hidden="true" className="shrink-0">
        <circle cx="6" cy="6" r="4.5" fill="none" stroke="currentColor" strokeWidth="1.6" opacity="0.35" />
        <circle
          cx="6"
          cy="6"
          r="4.5"
...
export function SectionShell({
  ...
  return (
    <section
      ...
      className={`${elevacion === 'elevada' ? 'bg-[var(--surface)] border-t border-[color-mix(in_oklab,var(--text-tertiary)_12%,transparent)]' : ''} ${pt} ${pb} ${className}`}
    >
...
export function CtaButton({
  ...
  return (
    <motion.a
      whileTap={{ scale: 0.97 }}
      href={href}
      className={`boton-principal relative inline-flex items-center justify-center overflow-hidden rounded-[var(--radius-button)] bg-[var(--accent)] px-8 text-[17px] font-semibold text-[var(--bg)] shadow-[0_8px_30px_color-mix(in_oklab,var(--accent)_25%,transparent)] outline-none transition-colors duration-150 hover:bg-[color-mix(in_oklab,var(--accent)_88%,var(--text-primary))] focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg)] [touch-action:manipulation] ${
        alto === 56 ? 'h-14' : 'h-[52px]'
      ...
      {visible && (
        ...
        >
          <motion.a
            ...
            className="boton-principal relative flex h-12 w-full items-center justify-center overflow-hidden rounded-[var(--radius-button)] bg-[var(--accent)] text-[16px] font-semibold text-[var(--bg)] outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg)] [touch-action:manipulation]"
          >
