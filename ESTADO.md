...
Diagnóstico real: NO hay riesgo de cuentas duplicadas (Supabase Auth ya garantiza un
correo = una cuenta) — el problema era que `sincronizarOnboardingSiHaceFalta`
(`lib/supabase/datos.ts`) descartaba el nombre nuevo EN SILENCIO cuando la cuenta ya
tenía uno (protección real, para no pisar ediciones manuales de Perfil — pero sin
avisar, confundía). Arreglado:
- La función ahora devuelve si la cuenta ya existía.
- `/app` (`app/app/page.tsx`) muestra un aviso descartable con el nombre real
  cuando alguien reaparece con un correo que ya tenía cuenta.
- El onboarding recién llenado se limpia de sessionStorage siempre (`limpiarOnboarding`
  en `lib/estado-app.ts`), sincronizado o no.
- El link "Entrar" del header de la landing ganó la aclaración "¿Ya tienes cuenta?"
  (visible desde `sm`) para que quien vuelve no repita el onboarding sin necesidad.
Verificado EN VIVO en producción con una cuenta de prueba descartable (creada y
borrada vía Admin API de Supabase, sin usar datos de nadie real): primer registro
con "Nombre Uno" → sin aviso, nombre correcto. Mismo correo, "Nombre Dos" en un
segundo onboarding → "Hola, Nombre Uno" se mantiene + aparece el aviso "Ese correo
ya tenía una cuenta..." + el botón de cerrarlo funciona + sessionStorage queda
limpio. tsc ✓ build ✓.
⚠️ Nota aparte, no tocada en este arreglo: el paywall sigue sin cobrar de verdad
(Hotmart pendiente) y `/app` no exige plan pago — ver auditoría de seguridad del
2026-09-09. No se agrava con este cambio, solo queda anotado de nuevo.
...
(`public/landing-app-*.jpg`) eran de ANTES del rediseño dorado/místico (una incluso de una versión
previa al pivote, con un mecanismo de "calma %" que ya no existe) — mostraban una app que ya no es
la real.
**Reemplazadas con capturas reales, sembradas con datos de ejemplo, del estado actual de la app**
(usuario de prueba temporal, borrado al terminar): `landing-app-situacion.jpg` (formulario de
situación con el selector de signo nuevo), `landing-app-cartas.jpg` (3 cartas reveladas invertidas),
`landing-app-lectura.jpg` (resultado de Compatibilidad — mismo par de signos que la imagen original,
Escorpio + Piscis, con el porcentaje difuminado A PROPÓSITO, igual que en el original, como gancho de
curiosidad), `landing-app-inicio.jpg` (Carta del día revelada + "Hoy te sentiste..." + Esta semana).
Procesadas con Python/Pillow: se recortaron 12px del borde derecho (una barra de scroll delgada que
dejaba la herramienta de captura) y se guardaron como `.jpg` optimizado con el MISMO nombre de
archivo que las originales — `app/page.tsx` no necesitó ningún cambio de código, el carrusel ya
apunta a esas rutas.
Verificado en vivo en `/`, deslizando el carrusel completo: las 4 tarjetas se ven correctas dentro
del marco de teléfono, sin la barra de scroll ni ningún corte raro. tsc/build limpios. Usuario de
prueba y los PNG temporales de captura borrados al terminar.
...
de los íconos de ánimo (checkpoint anterior, mismo día), pero en `components/app/SemanaStrip.tsx`:
el círculo de un día con ánimo registrado se rellenaba (`backgroundColor`) con el acento oscuro de
ese ánimo y el NÚMERO se pintaba `--bg` (casi negro) — círculo oscuro + número oscuro, se veía como
un borrón opaco. Se invirtió: el círculo ahora siempre es `--text-primary` (blanco/crema, mismo tono
que ya usa el círculo de "hoy"), y el color del ánimo pasa al NÚMERO — conserva la idea de "cada día
un color distinto" sin perder legibilidad.
⚠️ Al hacer el cambio apareció un contraste nuevo que no estaba antes: medido con la fórmula real de
WCAG, "tranquila" (dorado claro, `--accent`) como texto sobre el círculo casi blanco da 1.7:1 —
muy por debajo del mínimo de 4.5:1 (los otros 3 acentos, al ser oscuros, dan 10-11:1 sin problema).
Se agregó `inkEstado()` en `lib/animo.ts`: devuelve el mismo color para 3 de los 4 ánimos, y solo para
"tranquila" devuelve una versión oscurecida (`color-mix` con 45% negro, verificado en ~5:1) — sin
tocar `colorEstado()` original, que sigue usándose donde el dorado claro SÍ funciona bien (íconos,
"Hoy te sentiste...").
Verificado con un usuario de prueba real con los 4 ánimos sembrados en 4 días distintos de la semana:
los 4 números se leen claros y con su color distintivo sobre el círculo crema. tsc/build limpios.
Usuario y datos de prueba borrados al terminar.

✅ CHECKPOINT — Contraste corregido en los íconos de "¿Cómo te sientes hoy?" (Inicio), 2026-09-15, a
pedido del usuario ("hay unos que se ven oscuros"). Mismo bug de patrón ya visto y corregido 2 veces
hoy (Lecturas, Bienestar): 3 de los 4 acentos del check-in de ánimo (`lib/animo.ts` — esperanzada/
ansiosa/triste) son colores OSCUROS del tema místico, pensados para fondos de tarjeta con texto
claro encima — pero acá se dibujaban como color de ÍCONO sobre un círculo teñido del mismo color
(`color-mix` al 16%), quedando casi invisibles; solo "tranquila" (dorado, el único acento claro)
se veía bien. Corregido en `app/app/page.tsx` con el mismo patrón ya probado: círculo sólido
`var(--bg)` (casi negro) detrás, ícono a color completo encima — así el contraste no depende de si
el acento de esa categoría es claro u oscuro. Se aprovechó para subir un poco el tamaño (ícono 16→18,
círculo 36→40px) para que los 4 se sientan con más presencia.
Verificado con un usuario de prueba real a 375px: los 4 íconos (pluma dorada, amanecer ámbar, olas
azules, lluvia ciruela) se leen claros sobre su círculo oscuro. tsc/build limpios. Usuario de prueba
borrado al terminar.
...
✅ CORREGIDO el mismo día, a pedido del usuario: se quitó de la línea de Supabase en
`app/privacidad/page.tsx` la frase de que "por ahora también envía los correos de acceso" (ya no es
exacto — Resend es quien hace el envío SMTP real) y se agregó **Resend** como su propio ítem en la
lista NOMBRADA de subprocesadores (regla de `47-LEGAL-FISCAL-Y-PRIVACIDAD.md`), con su función real
("entrega el correo con tu enlace de acceso y otras comunicaciones de tu cuenta, nunca contenido de
tus lecturas"). Verificado en vivo en `/privacidad`. tsc/build limpios.
